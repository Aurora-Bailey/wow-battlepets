<template>
  <v-layout column justify-center align-center>
    <v-flex xs12 sm8 md6>
      ...
      <v-card>
        <v-card-title class="headline">Kai's Battle Pets</v-card-title>
        <v-card-text>
          <v-select :items="regions" v-model="region" label="Realm"></v-select>
  ...
  export default {
    ...
    methods: {
      async auctionListings (event) {
        let list = await this.$axios.$get(`http://54.244.210.52:3303/buy?ah=${this.auctionHouse}&region=${this.region}`)
